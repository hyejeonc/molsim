...
subroutine DrawAtom
      if(.not.allocated(icount)) then
         if (any(latweakcharge)) then
            allocate(icount(2*nat))
         else
            allocate(icount(nat))
         end if
      end if
      ...
      do ia = 1, na
         ...
         ! This is messy and should probably be improved
         ! 
         if (txelec == 'weakcharge') then
            if (latweakcharge(iat) .and. (.not. laz(ia))) cycle
         end if
         !if (.not.latweakcharge(iat) .or. laz(ia)) then  ! Fails on some tests when upper bound of laz == 0
            icount(icolor) = icount(icolor) + 1
            if (icount(icolor) == 1) then     ! first atom of a color
               write(unit,'(a,3f10.2)')   'Transform { translation ', vaux(1:3,ia)
               write(unit,'(a,a,a)')      ' children [ DEF ', trim(txcolor), ' Shape{ appearance Appearance { material'
               write(unit,'(a,3f10.2,a)') ' Material {diffuseColor ', rgbcolor(1:3,icolor), '}}'
               write(unit,'(a,f6.2,a)')   ' geometry Sphere { radius', atsize(iat), '} } ] }'
            end if
            write(unit,'(a,3f10.2,3a)') 'Transform { translation ', vaux(1:3,ia), ' children [ USE ', trim(txcolor), ']}'
         !end if
      end do

      
      if (.not. lgr .and. any(latweakcharge)) then
         icount = 0
         do ia = 1, na
            iat = iatan(ia)
            icolor = iat
            txcolor = txat(iat)
            if (latweakcharge(iat) .and. (.not. laz(ia)) ) then    ! neutral atoms of weak charge
               icount(icolor) = icount(icolor) + 1
               if (icount(icolor) == 1) then     ! first neutral atom - darker color
                  ! add '_N' to atom name as not all VRML-viewers (e.g. Chimera) support redefining variables
                  write(unit,'(a,3f10.2)')   'Transform { translation ', vaux(1:3,ia)
                  write(unit,'(a,a,a)')      ' children [ DEF ', trim(txcolor), '_N Shape{ appearance Appearance { material'
                  write(unit,'(a,3f10.2,a)') ' Material {diffuseColor ', rgbcolor(1:3,icolor)*0.5, '}}'
                  write(unit,'(a,f6.2,a)')   ' geometry Sphere { radius', atsize(iat), '} } ] }'
               end if
               write(unit,'(a,3f10.2,3a)') 'Transform { translation ', vaux(1:3,ia), ' children [ USE ', trim(txcolor), '_N ]}'
            endif
         enddo
      end if
...
! ... declare atoms
   ...
   if (lgr) then
      ...
      write(unit,'(a5,i5,a8,E13.5,a6,a11,a6,a,a8,E10.4)') &
         ('atom ',ia-1, &
          ' radius ',atsize(iatan(ia)), &
          ' type ',ReplaceText(trim(txat(iatan(ia))), " ", "_"), &
...
! ... determine structure file name
   ...
   if (lsplitvtf) then
      write(unit,'(a)') '   set filelist [lsort -dictionary [glob '//trim(adjustl(project))//'.*.vtf]]'
...
! ... draw frame according to the geometry of the simulation cell
   ...
   if (tximage(1) == 'frame') then
      ...
      if (lbcbox) then
         ...
         call Warn(txroutine,'Drawing of simulation cell currently only for lbcbox, lbcrd, lbcsph and lbccyl',uout)
      end if
   end if
...
subroutine DrawSphereTCL

  write(unit,'(a14,f8.1)') '   set sphrad ', sphrad
  write(unit,'(a)')        '   foreach i [list 0 78 157 235] {'
  write(unit,'(a)')        '      for {set j 0} {$j < 157} {incr j} {'
  write(unit,'(a)')        '         set x1 [expr { $sphrad*cos(0.01*$i)*sin(0.04*$j) }]'
  write(unit,'(a)')        '         set y1 [expr { $sphrad*sin(0.01*$i)*sin(0.04*$j) }]'
  write(unit,'(a)')        '         set z1 [expr { $sphrad*cos(0.04*$j) }]'
  write(unit,'(a)')        '         set x2 [expr { $sphrad*cos(0.01*$i)*sin(0.04*($j+1)) }]'
  write(unit,'(a)')        '         set y2 [expr { $sphrad*sin(0.01*$i)*sin(0.04*($j+1)) }]'
  write(unit,'(a)')        '         set z2 [expr { $sphrad*cos(0.04*($j+1)) }]' 
  write(unit,'(a)')        '         graphics $frameID line "$x1 $y1 $z1" "$x2 $y2 $z2" width 1 style dashed'

  write(unit,'(a)')        '         set x1 [expr { $sphrad*cos(0.04*$j)*sin(0.01*$i) }]'
  write(unit,'(a)')        '         set y1 [expr { $sphrad*sin(0.04*$j)*sin(0.01*$i) }]'
  write(unit,'(a)')        '         set z1 [expr { $sphrad*cos(0.01*$i) }]'
  write(unit,'(a)')        '         set x2 [expr { $sphrad*cos(0.04*($j+1))*sin(0.01*$i) }]'
  write(unit,'(a)')        '         set y2 [expr { $sphrad*sin(0.04*($j+1))*sin(0.01*$i) }]'
  write(unit,'(a)')        '         set z2 [expr { $sphrad*cos(0.01*$i) }]'
  write(unit,'(a)')        '         graphics $frameID line "$x1 $y1 $z1" "$x2 $y2 $z2" width 1 style dashed'
  write(unit,'(a)')        '      }'
  write(unit,'(a)')        '   }'

end subroutine DrawSphereTCL
  
! ......................................................................

subroutine DrawCylinderTCL

  write(unit,'(a11,f8.1)') '   set len ', Half*cyllen
  write(unit,'(a11,f8.1)') '   set rad ', cylrad
  write(unit,'(a)')        '   for {set i 0} {$i < 157} {incr i} {'
  write(unit,'(a)')        '      set x1 [expr { $rad*cos(0.04*$i) }]'
  write(unit,'(a)')        '      set y1 [expr { $rad*sin(0.04*$i) }]'
  write(unit,'(a)')        '      set x2 [expr { $rad*cos(0.04*($i+1)) }]'
  write(unit,'(a)')        '      set y2 [expr { $rad*sin(0.04*($i+1)) }]'
  write(unit,'(a)')        '      graphics $frameID line "$x1 $y1  $len" "$x2 $y2  $len" width 2 style solid'
  write(unit,'(a)')        '      graphics $frameID line "$x1 $y1 -$len" "$x2 $y2 -$len" width 2 style solid'
  write(unit,'(a)')        '   }'

  write(unit,'(a)')        '   foreach i {0.0 0.7854 1.5708 2.3562 3.1416 3.9270 4.7124 5.4978} {'
  write(unit,'(a)')        '      set x [expr { $rad*cos($i) }]'
  write(unit,'(a)')        '      set y [expr { $rad*sin($i) }]'
  write(unit,'(a)')        '      graphics $frameID line "$x  $y $len" "$x  $y -$len" width 1 style dashed'
  write(unit,'(a)')        '   }'
    
end subroutine DrawCylinderTCL
