...
!  3    in and out 
   ...
   integer(4)   , parameter :: ntype = 3  !ntype = 10 > 3
   ...
   integer(4) :: iseg, ic, ict, ip, ivar, ibin, itype
   real(8) ::  r2
   character(6) :: str1

   namelist /nmlCBOTC/ rcontact, vtype
   ...
   case (iReadInput)
      ...
      vtype%l     =.false.
      vtype%min   = [0.5, 0.5, 0.5]
      vtype%max   = [npct(ict)+0.5, npct(ict)+0.5, npct(ict)+0.5]
      vtype%bin   = nbin
           ...
           ict = ictcn(ic)
            do iseg = 1, npct(ict)
               ip = ipnsegcn(iseg,ic)
                  r2 = ro(1,ip)**2 + ro(2,ip)**2
print*,'ic, ict, ip, r2', ic, ict, ip, r2
                 if (((dcap-rcontact)**2 > r2) .or. (r2 > dcap**2)) cycle
...
! ... sample type = 3, inside and outside 
      itype = 3
        do ic = 1, nc
           ict = ictcn(ic)
            do iseg = 1, npct(ict)
