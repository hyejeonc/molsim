...
subroutine SetConfigurationUser(ipt, lsetconf)
   ...
   else if (txsetconf(ipt) =='random_cyl_shell') then
      call SetRandomAroundCylShell(ipt)
   else if (txsetconf(ipt) =='chainlinepma') then
      call SetChainLinePMA() !ipt is not needed
   else if (txsetconf(ipt) == 'randsurfpart') then
      call SetRandomSurfacePart(ipt)
   else if (txsetconf(ipt)(6:20) == 'nanotubesurface') then
      call SetNanotubeSurface(ipt)
...
!************************************************************************
!*                                                                      *
!*     SetRandomAroundCylShell                                          *
!*                                                                      *
!************************************************************************

! ... generate a random configuration around a hard cylinder shell

subroutine SetRandomAroundCylShell(ipt)

   use CoordinateModule
   implicit none
   integer(4), intent(in) :: ipt                   ! particle type

   character(40), parameter :: txroutine ='SetRandomAroundCylShell'
   integer(4) :: ntry, itry, ip, iploc
   real(8) :: rmin2, r2, rcyl, dcyl
   logical :: lWarnHCOverlap

   if (nppt(ipt) == 0) return

   ntry = 10*nppt(ipt)
!print*,'SetAroundCylShellCalled'
!print*,'rcap, dcap', rcap, dcap

   do iploc = 1, nppt(ipt)                                      ! loop over particles of type ipt
      ip = iploc-1+ipnpt(ipt)                                   ! particle to be set

!print*,'ipt, iploc, ip', ipt, iploc, ip
...
!print*,'coordinates, r2:', ro(1,ip), ro(2,ip), ro(3,ip), r2
        if (r2 <= (rcap+dcap+radat(ipt))**2.and. r2 >=(rcap-radat(ipt))**2) cycle  ! particle generated inside the shell - reject
         if (lWarnHCOverlap(ip, radatset, .true.)) cycle        ! check if atom-atom hard-core overlap
         lpset(ip) = .true.                                     ! configuration accepted
         exit
      end do
...
end subroutine SetRandomAroundCylShell
...
!************************************************************************
!*                                                                      *
!*        SetRandomSurfacePart                                          *
!*                                                                      *
!************************************************************************

! ... set particles of type ipt randomly on the surface of particles of type ihost
!     
!     

subroutine SetRandomSurfacePart(ipt)
   ...
   character(40), parameter :: txroutine ='SetRandomSurfacePart'
   integer(4) :: ntry, nset, itry, ip, ihost
   real(8)    :: dx, dy, dz, r2, norm
   logical    :: lWarnHCOverlap

   if (nppt(ipt)==0) return

   ntry =  ntrydef*nppt(ipt)    ! ntrydef (100) tries per particle of type ipt
   ihost = 1                ! host particle, currently first particle type
   nset = 1
   do itry=1,ntry

      !natpt(ipt)           ! number of atoms types in ipt
      !naatpt(1:natpt,ipt)  ! numper of atoms of a given atom type in particle ipt
      !rain
      
      ip=nset-1+ipnpt(ipt)

      call SphRandom(iseed,dx,dy,dz)   ! get random numbers on unit sphere

      call PBCr2(dx,dy,dz,r2)

      norm = (radat(iptpn(ihost)) + radat(ipt)) / sqrt(r2)
      ro(1,ip) = ro(1,ihost) + dx*norm
      ro(2,ip) = ro(2,ihost) + dy*norm
      ro(3,ip) = ro(3,ihost) + dz*norm

      call SetAtomPos(ip, ip, .false.)

      ! check if atom-atom hard-core overlap with other particles
      if (lWarnHCOverlap(ip, radatset, .true.))   cycle
      lpset(ip)=.true.                          ! trial configuration accepted
      if (nset==nppt(ipt)) exit                 ! check if finished
      nset=nset+1                               ! update nset

   end do

   if (itry>ntry) then            ! number of trial attempts exceeds the maximal one ?
      if (master) write(uout,'(i5,a,i5,a,i5,a,i5,a)') nset, 'of', nppt(ipt), 'particles of type', ipt,' set after', ntry, 'attempts'
      call Stop(txroutine, 'random configuration for surface particles failed, itry > ntry', uout)
   end if
   
 end subroutine SetRandomSurfacePart
...
! ... set particles of type ipt randomly on the inner or outer surface of nanotube
!     uses rcap, dcap and caplen for nanotube radius, thickness and length
!     the "cap" is from capside as we essentially have a cylindrical capside

 subroutine SetNanotubeSurface(ipt)
   ...
   character(40), parameter :: txroutine ='SetNanotubeSurface'
   integer(4) :: ntry, nset, itry, ip, ncircle, nlen, i, j
   real(8)    :: dx, dy, dz, d_sep, prad, r2, circumference, Random, dtheta, theta
   logical    :: lWarnHCOverlap

   
   if (nppt(ipt)==0) return

   if (txsetconf(ipt)(1:5) == 'inner') then
      prad = rcap-radat(ipt)
      r2 = prad-0.000001    ! small float to avoid round-off errors causing check in UExternalCylShell to fail
   else if (txsetconf(ipt)(1:5) == 'outer') then
      prad = rcap+dcap+radat(ipt)
      r2 = prad+0.000001
   else
      call Stop(txroutine, 'have not specified inner or outer surface of nanotube')
   end if

   circumference = 2*Pi*prad
   ncircle = sqrt(nppt(ipt)*circumference/lcap)

   !write(*,*) ncircle, mod(ncircle,3)
   
   if (mod(ncircle,3) == 1) then
      ncircle = ncircle + 2  ! -1 instead of +2?
   else if (mod(ncircle,3) == 2) then
      ncircle = ncircle + 1
   end if
   ncircle = ncircle + ncircle/3  ! ensures we don't overstep lcap, but gives denser configuration at start/end
   
   !write(*,*) mod(sqrt(nppt(ipt)*circumference/lcap), 3.0),  ncircle,  mod(ncircle,3)

   d_sep = circumference/ncircle
   
   nlen = 3*nppt(ipt)/(2*ncircle)   ! nlen = nppt(ipt) / (ncircle - ncircle/3) as every third particle in the circle is skipped

   ! dz is where we start to place charges in the z-direction, i.e. the offset from the edge of the cylinder
   if (nlen*ncircle < nppt(ipt)) then  ! possible due to integer flooring/ceiling, will have nlen+1 (partial) circles
      dz = -nlen*d_sep/2
   else
      dz = -(nlen-1)*d_sep/2
   end if
   ...
   !write(*,*) circumference, ncircle, nlen, d_sep, lcap/d_sep, nppt(ipt)/ncircle, nlen*ncircle
   if (nlen*(ncircle-1) /= nppt(ipt) ) then
      call Warn(txroutine, 'nlen*(ncircle-1) /= nppt(ipt), consider changing the number of particles', uout)
      !write(*,*) 'ipt:', ipt, '', 'nlen = ', nlen, 'ncircle = ', ncircle, 'nppt(ipt) = ', nppt(ipt), 'nlen*(ncircle-1) = ', nlen*(ncircle-1)
      !write(*,*) 'nlen*ncircle /= nppt(ipt)', nlen, ncircle, nlen*ncircle, nppt(ipt)
   end if
   ...
   do i=1, nlen+1
      theta = 0
      if (mod(i,2) == 0) theta = 3*d_sep/(2*prad)
      do j=1, ncircle
         ...
         ip=nset-1+ipnpt(ipt)
         ro(1,ip) = r2*cos(theta)
         ro(2,ip) = r2*sin(theta)
         ro(3,ip) = dz

         call SetAtomPos(ip, ip, .false.)
         
         
         if (nset==nppt(ipt)+1) exit
         nset = nset+1
         !write(*,*) ip, ro(:,ip)
      end do
      dz = dz+d_sep
   ...
   ! Place particles in circle, move circle. Have check for last particle. At first, just implement so last circle lacks the remaining particles if not enough
   ! Later, maybe change lcap instead, or have a routine which checks if it's even and if not reduce/increase the number of particles in the circle, at expense
   ! of number of circles. Adjust lcap accordingly.
   

 end subroutine SetNanotubeSurface
...
!        call SFPBC2DNoAv(iStage)
      ...
      if (txuser == 'weakpol') then    ! routines for analyses of NP-PE adsorption and weak charges
         !call MeanSegCharge(iStage)
         call MeanPartCharge(iStage)
         !call ChargeModulusDF(iStage)
         !call ComplexCharge(iStage)
         !call XAdsChains(iStage)
         !call MeanDist(iStage)
         !call ChainBeadPartContactBranched(iStage)
         if (lhierarchical) call AminoCharge(iStage)
      end if
...
!************************************************************************
!*                                                                      *
!*     CheckAdsChainOutTube                                             *
!*                                                                      *
!************************************************************************

! ... check if a chain and its segments are adsorbed on the nanotube - Hye 

subroutine CheckAdsChainOutTube(ic, adscond, ladschain, ladsseg)
   ...
   if (adscond%txobject == 'plane_xy') then   ! segment adsorbed according to its z-coordinate
      ...
      do iseg = 1, npchain                    ! loop over segments, check if segment is adsorbed
         ...
         if (adscond%txsurface == '-') then                   ! check lower surface
            ...
            call Stop('CheckAdsChainOutTube', 'error in adscond%txsurface', uout)
         end if
      end do
      ...
      do iseg = 1, npchain                    ! loop over segments, check if segment is adsorbed at jp
         ip = ipnsegcn(iseg,ic)
         dx = ro(1,ip)-ro(1,jp)               ! ip for chain, jp for particle 
         dy = ro(2,ip)-ro(2,jp)
         dz = ro(3,ip)-ro(3,jp)    
         call PBCr2(dx,dy,dz,r2)              ! molaux.F90            
         if (r2 < dist2) ladsseg(iseg) = .true.
      end do
   else if (adscond%txobject == 'tube_outer') then
      txno = adscond%txobject(6:8)
      read(txno,'(i3)') jp                    ! jp is the id of the "adsorbing" particle
      dist2 = adscond%dist**2
      do iseg = 1, npchain                    ! loop over segments, check if segment is adsorbed at jp
         ...
         r2=ro(1,ip)**2+ro(2,ip)**2
            if (((dcap+rcap)**2 > r2) .or. (r2 > (dcap+rcap+rcontact)**2)) cycle  
               if(ro(3,ip)**2 > (-lcap/2.0 - rcontact)) .and. (ro(3,ip)**2 < (lcap/2.0 + rcontact)) then 
                  ladsseg(iseg) = .true.
            end if   
      end do
   end if 
...
end subroutine CheckAdsChainOutTube

!************************************************************************
!*                                                                      *
!*     ChainBeadOutTubeCont                                             *
!*                                                                      *
!************************************************************************

! ... calculate probability of chain bead contact to the outer surface of a tube - Hye

   subroutine ChainBeadOutTubeCont(iStage)
   ...
   character(40), parameter :: txroutine ='ChainBeadOutTubeCont'
   character(80), parameter :: txheading ='probability of chain bead-outer tube cont'
...
module StorageMSC
  ...
  integer(4), parameter :: ntype = 3      !number of types of properties
  integer(4), parameter :: mnbin  = 200
  integer(4) :: mnvar
  character(5),  save :: txtype(ntype) = ['Zmott', 'Zmo1m','Zmo0m']

  logical,       allocatable :: vtype(:,:)
  real(8),       allocatable :: vmin(:,:), vmax(:,:)
  integer(4),    allocatable :: nvar(:), nbin(:)!, nadsmic(:)
  integer(4),    allocatable :: ipnt(:,:)
  type(df_var),  allocatable :: var(:,:)
  integer(4),    allocatable :: nnonzero(:,:,:)  
  integer(4),    allocatable :: norm_counter(:,:,:)   ! For normalization of var%avs2


contains

  subroutine allocMSCArrays()

    mnvar  = ntype*nc  !max no of variables
    if (.not.allocated(vtype)) then
       allocate(vtype(nct, ntype))
       allocate(vmin(nct, ntype))
       allocate(vmax(nct, ntype))
       allocate(nvar(nct))
       allocate(ipnt(ntype, nct))
       allocate(nbin(nct))
       !allocate(nadsmic(nc))
       allocate(var(ntype, nct))
       allocate(norm_counter(nct, ntype, -1:mnbin))
       allocate(nnonzero(nct, mnvar,0:mnbin))
    end if

  end subroutine allocMSCArrays

  subroutine deallocMSCArrays()
    ...
    if (allocated(vtype)) then
       ...
       !deallocate(nadsmic)
       deallocate(var)
       deallocate(norm_counter)
       deallocate(nnonzero)
    end if

  end subroutine deallocMSCArrays

end module StorageMSC
...
! ... calculates the mean charge of each monomer of a polyion of a given type
! ... for weak polyelectrolytes - Rita

!     parallel version (master only)


! subroutine MeanSegCharge(iMode)
!   use MolModule
!   use StatisticsModule
!   use StorageMSC
!   implicit none
...
!   character(40), parameter :: txroutine = 'MeanSegCharge'
!   character(60), parameter :: txheading = 'Average charge of PE monomers'
!   character(60)            :: listheading

!   type(adscond_var), save :: adscond  ! Adsorption condition/threshold
!   logical             :: ladschain, ladsseg(64)              ! For passing to CheckAdsChainSeg
!   logical             :: lfirst
!   integer(4)          :: ivar, ibin, itype
!   integer(4)          :: ip, ipt, ic, ict, iseg
!   real(8)             :: qhelp

!   namelist /nmlMeanSegCharge/ vtype, adscond

!   if(ltrace) call WriteTrace(2, txroutine, iMode)

!   if(slave) return

!   call CpuAdd('start', txroutine, 1, uout)

!   if(iMode==iReadInput) then

!      call allocMSCArrays()
!      ! ... read  vmin, vmax, rmax, nbin

!      do ict = 1, nct

!         vtype(ict, 1:ntype) = .false.
!         vmin(ict, 1:ntype) = 0.5
!         vmax(ict, 1:ntype) = npct(ict)+0.5
!         nbin(ict)    = npct(ict)

!         if(nbin(ict)>mnbin) call stop(txroutine,'nbin>mnbin',uout)

!      enddo

!      rewind(uin)
!      read(uin,nmlMeanSegCharge)

!      call LowerCase(adscond%txobject)
!      call LowerCase(adscond%txsurface)

!   elseif(iMode==iWriteInput) then

!      do ict = 1, nct
!         nvar(ict)=0

!         do itype = 1, ntype
!            if(vtype(ict, itype)) then
!               nvar(ict)=nvar(ict)+1

!               if(nvar(ict) > mnvar) call Stop(txroutine, 'nvar(ict) > mnvar', uout)

!               ipnt(itype,ict) = nvar(ict)

!               var(nvar(ict), ict)%min = vmin(ict, itype)
!               var(nvar(ict), ict)%max = vmax(ict, itype)
!               var(nvar(ict), ict)%nbin = nbin(ict)

!               var(nvar(ict), ict)%label = '  ' // trim(txtype(itype))
...
!      do ict = 1, nct
!         nvar(ict) = count(vtype(ict,:))
!         call DistFuncSample(iMode, nvar(ict), var(:,ict))
!      enddo

!   else if(iMode == iBeforeSimulation) then

!      do ict = 1, nct
!         nnonzero(ict,1:mnvar,0:nbin(ict)) = 0
!         nvar(ict) = count(vtype(ict,:))
!         call DistFuncSample(iMode, nvar(ict), var(:,ict))

!         if(lsim .and. master .and. txstart == 'continue') read(ucnf) var(1:nvar(ict), ict)
!         if(lsim .and. master .and. txstart == 'continue') read(ucnf) norm_counter(ict, 1:nvar(ict), -1:mnbin)
!         if(lsim .and. master .and. txstart == 'continue') read(ucnf) nnonzero(ict,1:mnvar,0:nbin(ict))
!      enddo

!   else if(iMode == iBeforeMacrostep) then
!      do ict = 1, nct
!         nvar(ict) = count(vtype(ict,:))
!         norm_counter(ict,:,:) = 0
!         call DistFuncSample(iMode, nvar(ict), var(:,ict))
...
!   else if(iMode == iSimulationStep) then

!      ! Note: az = atom charge. laz = logical atom charge (.true. if charged)
!      !       var, var_norm of type df_var (statistics.F90)
!      !       df_var%avs2 = avg. of a macrostep. df_var%nsamp2 = no. of values sampled per macrostep

!      do ict = 1, nct
!         nvar(ict) = count(vtype(ict,:))
!         var(1:nvar(ict), ict)%nsamp2 = var(1:nvar(ict), ict)%nsamp2+1
!      enddo


!      ! Run through chain types, each chain of the type and chain segments. Chain segment loop is done twice -
!      ! once for the total average (Zmott) and once for chains adsorbed/not adsorbed. 
!      do ict = 1, nct
!         do ic = icnct(ict), icnct(ict) + ncct(ict) - 1

!            itype = 1            
!            if (vtype(ict, 1)) then
!               do iseg = 1, npct(ict) ! Runs through monomers/particles anyway, change from segments to make clear?
!                  ! Currently no way to define segment size in namelist. Include later?
!                  ip = ipnsegcn(iseg,ic)
!                  qhelp = 0.0
!                  if(laz(ip)) qhelp = 1.0

!                  ivar = ipnt(itype,ict)
!                  ibin = max(-1,min(int(var(ivar, ict)%bini*(iseg-var(ivar, ict)%min)),var(ivar, ict)%nbin))
!                  var(ivar, ict)%avs2(ibin) = var(ivar, ict)%avs2(ibin)+abs(qhelp*az(ip))
!                  norm_counter(ict, ivar, ibin) = norm_counter(ict, ivar, ibin) + 1

!               enddo
!            endif

!            !call TagChainAdsMic(ic, adscond, nadsmic)
!            ladschain = .false.
!            ladsseg = .false.
!            call CheckAdsChainSeg(ic, adscond, ladschain, ladsseg)
!            if (vtype(ict, 2) .and. ladschain) itype = 2
!            if (vtype(ict, 3) .and. .not. ladschain) itype = 3
!            if (itype > 1) then
!               do iseg = 1, npct(ict)   
!                  ip = ipnsegcn(iseg,ic)
!                  qhelp = 0.0
...
!   else if(iMode == iAfterMacrostep) then

!      ! var%avs* initialised to zero in DistFuncSample
!      ! Should not call DistFuncSample after macrostep, due to different normalization
!      do ict = 1, nct
!         nvar(ict) = count(vtype(ict,:))
!         do ivar = 1, nvar(ict)
!            do ibin = 0, nbin(ict)
!               if(norm_counter(ict, ivar, ibin) == Zero)cycle
!               var(ivar, ict)%avs2(ibin)=var(ivar, ict)%avs2(ibin)/norm_counter(ict, ivar, ibin)

!               if(norm_counter(ict, ivar, ibin) <= 0)cycle
!               var(ivar, ict)%avs1(ibin) = var(ivar, ict)%avs1(ibin) + var(ivar, ict)%avs2(ibin)
!               var(ivar, ict)%avsd(ibin) = var(ivar, ict)%avsd(ibin) + var(ivar, ict)%avs2(ibin)**2
!               nnonzero(ict,ivar,ibin) = nnonzero(ict,ivar,ibin) + 1
!               ! ibin = 0 -> problems with array access for nnonzero
!            enddo
!         enddo

!         if(lsim .and. master) write(ucnf) var(1:nvar(ict), ict)
!         if(lsim .and. master) write(ucnf) nnonzero(ict,1:mnvar,1:nbin(ict))
!         if(lsim .and. master) write(ucnf) norm_counter(ict, 1:nvar(ict), -1:mnbin)

!      enddo

!   else if(iMode == iAfterSimulation) then

!      do ict = 1, nct
!         do ivar = 1, nvar(ict)
!            do ibin = 0,nbin(ict)

!               if(nnonzero(ict,ivar,ibin) == 0)cycle
!               var(ivar, ict)%avs1(ibin) = var(ivar, ict)%avs1(ibin)/nnonzero(ict,ivar,ibin)
!               var(ivar, ict)%avsd(ibin) = max(Zero,var(ivar, ict)%avsd(ibin)&
!                    -var(ivar, ict)%avs1(ibin)**2*nnonzero(ict,ivar,ibin))/(nnonzero(ict,ivar,ibin)-1)
!               if(nnonzero(ict,ivar,ibin) == 1) var(ivar, ict)%avsd(ibin)=0.0d0
!            enddo
!         enddo
!      enddo
...
!      do ict = 1, nct
!         if (lfirst) then
!            listheading = (trim(txheading) // NEW_LINE('') // '  ' // trim(txct(ict)))
!            lfirst = .false.
!         else
!            listheading = '  ' // trim(txct(ict))
!         endif

!         call DistFuncHead(nvar(ict), var(:,ict), uout)
!         call DistFuncWrite(listheading, nvar(ict), var(:,ict), uout, ulist, ishow, iplot, ilist)
!      enddo
     
!      call deallocMSCArrays()
!   end if

!   call CpuAdd('stop', txroutine, 1, uout)


! end subroutine MeanSegCharge
...
! ... calculates the mean charge of each particle of a given type. For use e.g. with annealed silica particles.
!     Does not separate between chains adsorbed to different number of nanoparticles.
!     Any need for adsorption conditions? E.g. particles adsorbed to chains - Morten



subroutine MeanPartCharge(iMode)
  use MolModule
  use StatisticsModule
  implicit none
  ...
  character(40), parameter :: txroutine = 'MeanPartCharge'
  character(60), parameter :: txheading = 'Average charge of particles'

  integer(4), parameter :: mnbin  = 600

  logical,       save :: vtype(mnpt)

  real(8),       save :: vmin(mnpt), vmax(mnpt)
  integer(4),    save :: nbin(mnpt), nvar, ipnt(mnpt)
  integer(4),    save :: nnonzero(mnpt,0:mnbin)
  type(df_var),  save :: var(mnpt), var2(mnpt)                      ! TODO: Array, one for each chain type?
  integer(4),    save :: norm_counter(mnpt, -1:mnbin)   ! For normalization of var%avs2
  integer(4)          :: i, ibin, ip, ipl, ipt
  real(8)             :: qhelp

  namelist /nmlMeanPartCharge/ vtype
  ...
  if(iMode==iReadInput) then
     ...
     vmin(1:npt) = 0.5
     do ipt = 1, npt
        vmax(ipt) = nppt(ipt)+0.5
        nbin(ipt) = nppt(ipt)

        if(nbin(ipt)>mnbin) call stop(txroutine,'nbin>mnbin',uout)
     enddo

     rewind(uin)
     read(uin,nmlMeanPartCharge)
     nvar = count(vtype)          ! number of particle types to check
     if (nvar > npt) call stop(txroutine, 'nvar > npt', uout)
  ...
  elseif(iMode==iWriteInput) then
     ...
     do ipt = 1, npt
        if(vtype(ipt)) then
           ...
           var(i)%label = '  ' // trim(txpt(ipt))

           var2(i)%min = vmin(ipt)
           var2(i)%max = vmax(ipt)
           var2(i)%nbin = nbin(ipt)

           var2(i)%label = '  ' // trim(txpt(ipt))

        endif
     enddo

     call DistFuncSample(iMode, nvar, var)
     !call DistFuncSample(iMode, nvar, var2)

  else if(iMode == iBeforeSimulation) then

     nnonzero(1:nvar,0:mnbin) = 0

     call DistFuncSample(iMode, nvar, var)
     !call DistFuncSample(iMode, nvar, var2)

     if(lsim .and. master .and. txstart == 'continue') read(ucnf) var(1:nvar)  ! if not lsim, what about lana or lmix?
     if(lsim .and. master .and. txstart == 'continue') read(ucnf) norm_counter(1:nvar, -1:mnbin)
     if(lsim .and. master .and. txstart == 'continue') read(ucnf) nnonzero(1:nvar,0:mnbin)
     ...
     var(1:nvar)%nsamp2 = var(1:nvar)%nsamp2+1
     !var2(1:nvar)%nsamp2 = var2(1:nvar)%nsamp2+1

     ! Run through particle types, and each particle of the type.
     i = 0
     do ipt = 1, npt
        if (vtype(ipt)) then

           i = i + 1
           do ipl = 1, nppt(ipt)  ! particle number, local (to the particle type)

              ip = ipnpt(ipt) + ipl - 1   ! particle number

              qhelp = 0.0
              if(laz(ip)) qhelp = 1.0

              ibin = max(-1,min(int(var(i)%bini*(ipl-var(i)%min)),var(i)%nbin))
              var(i)%avs2(ibin) = var(i)%avs2(ibin)+abs(qhelp*az(ip))
              norm_counter(i, ibin) = norm_counter(i, ibin) + 1

              !var2(i)%avs2(ibin) = var2(i)%avs2(ibin)+abs(qhelp*az(ip))
           enddo
        endif
     enddo
     ...
     ! var%avs* initialised to zero in DistFuncSample
     ! Do not call DistFuncSample in MeanSegCharge due to different normalization
     ! for Zmo1m and Zmo0m. Should be the same here.
     ! Standard deviation is calculated differently in iAfterSim though

     !call DistFuncSample(iMode, nvar, var2)
     do i = 1, nvar
        do ibin = 0, var(i)%nbin
           if(norm_counter(i, ibin) == Zero)cycle
           var(i)%avs2(ibin)=var(i)%avs2(ibin)/norm_counter(i, ibin)

           if(norm_counter(i, ibin) <= 0)cycle
           var(i)%avs1(ibin) = var(i)%avs1(ibin) + var(i)%avs2(ibin)
           var(i)%avsd(ibin) = var(i)%avsd(ibin) + var(i)%avs2(ibin)**2
           nnonzero(i,ibin) = nnonzero(i,ibin) + 1
        enddo
     enddo
     if(lsim .and. master) write(ucnf) var(1:nvar)
     if(lsim .and. master) write(ucnf) nnonzero(1:nvar,0:mnbin)
     if(lsim .and. master) write(ucnf) norm_counter(1:nvar, -1:mnbin)
     ...
     do i = 1, nvar
        do ibin = 0, var(i)%nbin
           ...
           if(nnonzero(i,ibin) == 0)cycle
           var(i)%avs1(ibin) = var(i)%avs1(ibin)/nnonzero(i,ibin)
           var(i)%avsd(ibin) = max(Zero,var(i)%avsd(ibin)&
                -var(i)%avs1(ibin)**2*nnonzero(i,ibin))/(nnonzero(i,ibin)-1)
           if(nnonzero(i,ibin) == 1) var(i)%avsd(ibin)=0.0d0
        enddo
     enddo
     ...
     !call DistFuncHead(nvar, var2, uout)
     !call DistFuncWrite(txheading, nvar, var2, uout, ulist, ishow, iplot, ilist)
...
end subroutine MeanPartCharge



module StorageCModDF
  use MolModule
  implicit none
...
contains
  ...
  subroutine allocCModDFArrays()
    ...
    if (.not.allocated(vtype)) then
       ...
       allocate(vmax(nct, ntype))
       allocate(nvar(nct))
       allocate(ipnt(ntype, nc))
       allocate(nbin(nct))
       allocate(var(ntype, nc))
    end if

  end subroutine allocCModDFArrays

  subroutine deallocCModDFArrays()
  ...
  end subroutine deallocCModDFArrays

end module StorageCModDF
...
! ... calculates the charge modulus in a polyion
! ... for weak polyelectrolytes - Rita
! ... currently only for adsorption to zero or one particle

!     parallel version (master only)


subroutine ChargeModulusDF(iStage)
  use MolModule
  use StatisticsModule
  use StorageCModDF
  implicit none
  ...
  character(40), parameter :: txroutine = 'ChargeModulusDF'
  character(60), parameter :: txheading = 'Charge modulus, weak PE'
  character(60) :: listheading
  type(adscond_var),          save :: adscond
  real(8),       save :: adsoffset
  integer(4)          :: ivar, ibin, itype
  integer(4)          :: ip, ipt, ic, iseg, ict
  real(8)             :: zpe, norm, norm1, normhelp, fac, vsum, qhelp
  logical             :: ladschain, ladsseg(64)        ! For passing to CheckAdsChainSeg
  logical             :: lfirst

  namelist /nmlChargeModulusDF/ vtype, adscond
  ...
  if(iStage==iReadInput) then

     call allocCModDFArrays()
     ! ... read  vmin, vmax, rmax, nbin
     do ict = 1, nct
     ...
     rewind(uin)
     read(uin,nmlChargeModulusDF)

     call LowerCase(adscond%txobject)
     call LowerCase(adscond%txsurface)


  elseif(iStage==iWriteInput) then
     ...
     do ict = 1, nct
        ...
        do itype = 1, ntype
           if(vtype(ict, itype)) then
              ...
              do ic = icnct(ict), icnct(ict) + ncct(ict) - 1
                 ipnt(itype,ic) = nvar(ict)

                 var(nvar(ict), ic)%min = vmin(ict, itype)
                 var(nvar(ict), ic)%max = vmax(ict, itype)
                 var(nvar(ict), ic)%nbin = nbin(ict)

                 var(nvar(ict), ic)%label = '  ' // trim(txtype(itype))
              enddo
     ...
     do ic = 1, nc
        ict = ictcn(ic)
        nvar(ict) = count(vtype(ict,:))
        call DistFuncSample(iStage, nvar(ict), var(:,ic))
     enddo



  else if(iStage == iBeforeSimulation) then
     ...
     do ic = 1, nc
        ...
        if(lsim .and. master .and. txstart == 'continue') read(ucnf) var(ic, 1:nvar(ict)) 
     enddo

  else if(iStage == iBeforeMacrostep) then
  ...
  else if(iStage == iSimulationStep) then

     do ic = 1, nc
        ict = ictcn(ic)
        nvar(ict) = count(vtype(ict,:))
        var(1:nvar(ict), ic)%nsamp2 = var(1:nvar(ict), ic)%nsamp2+1
     enddo
     ...
     do ict = 1, nct
        do ic = icnct(ict), icnct(ict) + ncct(ict) - 1
           ...
           if (vtype(ict, 1)) then
              ...
              do iseg = 1, npct(ict)
                 ip = ipnsegcn(iseg,ic)
                 if(laz(ip)) zpe = zpe + az(ip)
              enddo

              ivar = ipnt(itype,ic)
              ibin = max(-1,min(int(var(ivar, ic)%bini*(abs(zpe)-var(ivar, ic)%min)),var(ivar, ic)%nbin))
              var(ivar, ic)%avs2(ibin) = var(ivar, ic)%avs2(ibin)+One
           endif
           ...
           if (vtype(ict, 2) .and. ladschain) itype = 2
           if (vtype(ict, 3) .and. .not. ladschain) itype = 3
           !call TagChainAdsMic(ichelp, adscond, nadsmic)
           if (itype > 1) then
  ...
  else if(iStage == iAfterMacrostep) then

     do ic = 1, nc
        ict = ictcn(ic)
        call DistFuncNorm(1, nvar(ict), var(:,ic))
        call DistFuncSample(iStage, nvar(ict), var(:,ic))

        if(lsim .and. master) write(ucnf) var(1:nvar(ict), ic)
     enddo

  else if(iStage == iAfterSimulation) then

     ! Normalizes over the macrosteps where there is occurences (when the chain adsorbs to a particular number of chains. So the sum of the Prob. is not necessarily 1. Leave it for now. 
     !call DistFuncHead(nvar(1), var(1,:), uout)
     !call DistFuncWrite(txheading, nvar(1), var(1,:), uout, ulist, ishow, iplot, ilist)
     call WriteHead(2, txheading, uout)
     lfirst = .true.
     do ict = 1, nct
               
        call DistFuncHead(nvar(ict), var(:,icnct(ict)), uout)
        do ic = icnct(ict), icnct(ict) + ncct(ict) - 1
           ...
           call DistFuncSample(iStage, nvar(ict), var(:,ic))
           call DistFuncWrite(listheading, nvar(ict), var(:,ic), uout, ulist, 0, 0, ilist)  !listsim           
           call DistFuncWrite(txct(ict), nvar(ict), var(:,ic), uout, ulist, ishow, iplot, 0) !outsim
           call DistFuncAverValue(nvar(ict), var(:,ic), uout)

        enddo
     enddo

     call deallocCModDFArrays()
  end if

  call CpuAdd('stop', txroutine, 1, uout)


end subroutine ChargeModulusDF
...
! Calculates the charge of a complex consisiting of 1 nanoparticle, with
! adsorbed polyions and counterions. Write out mean charge and charge distribution
...
module StorageCC
  ...
  logical,  allocatable  :: pincomplex(:)

contains

  subroutine StorageCCAlloc(n)
    integer, intent(in) :: n
    if(.not.allocated(pincomplex)) allocate(pincomplex(n))
  end subroutine StorageCCAlloc


  subroutine StorageCCDealloc()
    if(allocated(pincomplex)) deallocate(pincomplex)
  end subroutine StorageCCDealloc


  subroutine PartInComplex(adscond)

    ! checks if particles are part of a complex with a base adsorbing particle given by adscond
    ...
    character(3)         :: txno
    integer(4)           :: ic, ict, ip, jp, iseg, cbp, up
    real(8)              :: dx, dy, dz, r2, dist2
    logical              :: updated
    type(adscond_var),intent(in) :: adscond

    if (adscond%txobject(1:4) == 'part') then
       txno = adscond%txobject(6:8)
       read(txno,'(i3)') cbp                    ! cbp (complex base particle) is the id of the "adsorbing" particle
    else
       call Stop('Pincomplex', 'error in adscond%txobject', uout)
    end if

    do ip = 1, np
       pincomplex(ip) = .false.
    end do
    pincomplex(cbp) = .true.  ! adsorbing particle is part of complex


    dist2 = adscond%dist**2

    ! icnpn(ip): particle ip -> its chain. 0 if ip is not part of a chain
    ! check if particles are adsorbed on NP - if so, they're part of the complex
    do ip = 1, np
       if(pincomplex(ip)) cycle

       dx = ro(1,ip)-ro(1,cbp)
       dy = ro(2,ip)-ro(2,cbp)
       dz = ro(3,ip)-ro(3,cbp)
       call PBCr2(dx,dy,dz,r2)
       if (r2 < dist2) pincomplex(ip) = .true.
       if(pincomplex(ip) .and. icnpn(ip) > 0) then  ! if part of a chain, add chain to complex
          ic = icnpn(ip)
          ict = ictcn(ic)
          do iseg = 1, npct(ict)
             pincomplex( ipnsegcn(iseg,ic) ) = .true.
          end do
       end if
    end do


    ! check if free particles are adsorbed to other particles in the complex
    if(count(pincomplex(1:np)) > 1) updated = .true.
    do while(updated)
       updated = .false.
       do ip = 1, np
          do jp = ip+1, np

             if(pincomplex(ip) .eqv. pincomplex(jp) .or. ip == cbp .or. jp == cbp) cycle
             ...
             if (r2 < dist2) then
                up = ip  ! updated particle, i.e. the one initially not counted as part of the complex
                if(.not.pincomplex(jp)) up = jp
                if(icnpn(up) > 0) then  ! if updated particle is part of a chain, add chain to complex
                   ic = icnpn(up)
                   ict = ictcn(ic)
                   do iseg = 1, npct(ict)
                      pincomplex( ipnsegcn(iseg,ic) ) = .true.
  ...
  end subroutine PartInComplex

end module StorageCC



! Count number of adsorbed chains (of each type) and counterions
! Separate adsorption threshold for counterions?
! In case of multilayers, polyions may adsorb on complex on top of other polyions,
! even though they're outside of the adsorption threshold of the NP.

! nbin: max is total possible charge

subroutine ComplexCharge(iStage)
  ...
  character(15), parameter :: txroutine = 'ComplexCharge'
  character(60), parameter :: txheading = 'Charge of NP-PE complex, w. CI'
  type(adscond_var),save :: adscond
  type(df_var),     save :: zcomplexdf
  integer(4)             :: ibin
  integer(4)             :: ip, ipt, ic
  real(8)                :: zmin, zmax, zc, zc2



  namelist /nmlComplexCharge/ adscond
  ...
  select case(iStage)    
  case(iReadInput)
     ! allocate arrays, read input to namelist

     call StorageCCAlloc(np)

     rewind(uin)
     read(uin,nmlComplexCharge)
  ...
  case(iWriteInput)
     ...
     zmax = 0.0
     do ipt = 1, npt
        if(zat(ipt) < 0) zmin = zmin + zat(ipt)*nppt(ipt)
        if(zat(ipt) > 0) zmax = zmax + zat(ipt)*nppt(ipt)
     end do
     zcomplexdf%min = zmin-0.5
     zcomplexdf%max = zmax+0.5
     zcomplexdf%nbin = zmax-zmin+1
     zcomplexdf%label = txroutine     

     call DistFuncSample(iStage, 1, zcomplexdf)

  case(iBeforeSimulation)
     !
     call DistFuncSample(iStage, 1, zcomplexdf)

     if(lsim .and. master .and. txstart == 'continue') read(ucnf) zcomplexdf
  ...
  case(iSimulationStep)
     ...
     call PartInComplex(adscond)

     zc = 0.0
     do ip = 1, np
        if(pincomplex(ip) .and. laz(ip)) zc = zc + az(ip)
     end do

     ibin = max(zmin-1, 1.0*min(int(zcomplexdf%bini*(zc - zcomplexdf%min)), zcomplexdf%nbin) )
     zcomplexdf%avs2(ibin) = zcomplexdf%avs2(ibin) + 1
     zcomplexdf%nsamp2 = zcomplexdf%nsamp2 + 1

     ! do ic = 1, nc
     !    if(count(pincomplex(fp:lp) > 0))  cincomplex(ictcn(ic)) = cincomplex(ictcn(ic)) + 1           
     ! end do
  ...
  case(iAfterMacrostep)
     ...
     if(lsim .and. master) write(ucnf) zcomplexdf

  case(iAfterSimulation)

     call DistFuncNorm(1, 1, zcomplexdf)
     call DistFuncSample(iStage, 1, zcomplexdf)

     call WriteHead(2, txheading, uout)
     call DistFuncAverValue(1, zcomplexdf, uout)
     call DistFuncHead(1, zcomplexdf, uout)     
     call DistFuncWrite(txheading, 1, zcomplexdf, uout, ulist, ishow, iplot, ilist)

     call StorageCCDealloc()

  end select

end subroutine ComplexCharge


subroutine ChainsInComplex(iStage)
  ...
  character(40), parameter :: txroutine ='ChainsInComplex'
  character(80), parameter :: txheading ='Fraction of chain type in complex'
  type(adscond_var), save  :: adscond
  type(scalar_var), allocatable, save :: var(:)              ! variable
  integer(4), save         :: ic, ict, iseg

  namelist /nmlChainsInComplex/ adscond
  ...
  if(iStage == iReadInput) then
     ...
     rewind(uin)
     read(uin,nmlChainsInComplex)

     call LowerCase(adscond%txobject)
     call LowerCase(adscond%txsurface)

  else if(iStage == iWriteInput) then
     ...
     call ScalarSample(iStage, 1, nct, var)
     if(lsim .and. master .and. txstart == 'continue') read(ucnf) var!(1:nct)
     ...
     ! ... sample the number of chains adsorbed micelle
     ...
     do ic = 1, nc
        ict = ictcn(ic)
        if(pincomplex(ipnsegcn(1, ic)))  var(ict)%value = var(ict)%value + 1.0
     end do
     do ict = 1, nct
        var(ict)%value = var(ict)%value/ncct(ict) ! fraction
     end do
     ...
     call ScalarSample(iStage, 1, nct, var)
     if(lsim .and. master) write(ucnf) var(1:nct)
     ...
     write(uout,'(a,t40,a    )') 'adsorbing object               = ', adscond%txobject
     write(uout,'(a,t35,f10.3)') 'adsorbing distance             = ', adscond%dist
     !      write(uout,'(a,t35,3f10.4)') 'position of micelle 1             = ', r(1:3,241)
     !      write(uout,'(a,t35,3f10.4)') 'position of micelle 2             = ', r(1:3,242)
     !      do ict = 1, nct
     !      ioffset = ntype*(ict-1)
     !    write(uout, '()')
     !    if (nct > 1) write(uout,'(2a)') 'chain: ', txct(ict)
     !    if (nct > 1) write(uout,'(a)')  '------------------'
     call ScalarWrite(iStage, 1, nct, var, 1, '(a,t35,4f15.3,f15.0)', uout)
     !     enddo
  ...
  call CpuAdd('stop', 'ChainsInComplex', 1, uout)

end subroutine ChainsInComplex


 
subroutine XAdsChains(iStage)

  ! calculates the fraction of chains adsorbed to a particle type
  ...
  character(40), parameter :: txroutine ='XAdsChains'
  character(80), parameter :: txheading ='Fraction of chain type adsorbed to a specific particle type'
  type(scalar_var), allocatable, save :: fca(:)    ! fraction of chains adsorbed
  integer(4), save         :: adspt, npa
  real(8), save            :: adsdist
  integer(4)               :: ic, ict, ip, iseg, ipa
  character(7)             :: temp_string

  namelist /nmlXAdsChains/ adspt, adsdist
  ! adspt: particle type to adsorb to
  ! adsdist: adsorption distance
  ...
  if(iStage == iReadInput) then
     ...
     rewind(uin)
     read(uin,nmlXAdsChains)


  else if(iStage == iWriteInput) then

     npa = nppt(adspt)+1   ! number of particles to adsorb on + 1 for zero adsorption
     allocate(fca(npa*nct))

     do ict = 1, nct
        do ipa = 1, npa
           write(temp_string, '(a6,i1)') 'X_ads_', ipa-1
           fca( (ict-1)*npa + ipa )%label = temp_string !//' ('//trim(txct(ict))//')'
        end do
     end do
     ...
     call ScalarSample(iStage, 1, nct*npa, fca)
     if(lsim .and. master .and. txstart == 'continue') read(ucnf) fca(:)
     ...
     ! sample the number of chains adsorbed
     ! run through each chain for all particles of a given type

     fca%value = 0

     do ic = 1, nc
        ict = ictcn(ic)
        ipa = 1   ! int, number of particles + 1 the chain is adsorbed to
        do ip = ipnpt(adspt), ipnpt(adspt) + nppt(adspt)-1
           if(ChainIsAdsorbed(ic, ip, adsdist)) then
              ipa = ipa + 1
           end if
        end do
        fca( (ict-1)*npa+ipa )%value = fca( (ict-1)*npa+ipa )%value + 1.0
     end do

     do ict = 1, nct
        do ipa = 1, npa
           fca((ict-1)*npa+ipa)%value = fca((ict-1)*npa+ipa)%value/ncct(ict) ! fraction
        end do
     end do
     ...
     call ScalarSample(iStage, 1, nct*npa, fca)
     if(lsim .and. master) write(ucnf) fca
     ...
     call WriteHead(2, txheading, uout)
     write(uout,'(a,t40,a    )') 'adsorbing particle type        = ', trim(txpt(adspt))
     write(uout,'(a,t35,f10.3)') 'adsorption distance            = ', adsdist
     !      write(uout,'(a,t35,3f10.4)') 'position of micelle 1             = ', r(1:3,241)
     !      write(uout,'(a,t35,3f10.4)') 'position of micelle 2             = ', r(1:3,242)

     do ict = 1, nct
        write(uout, '(a)') ''
        write(uout,'(2a)') 'chain type: ', trim(txct(ict))
        call ScalarWrite(iStage, (ict-1)*npa+1, ict*npa, fca((ict-1)*npa+1:ict*npa), 1, '(a,t35,4f15.3,f15.0)', uout)
        write(uout,'(a)')  '--------'
     end do
  ...
  call CpuAdd('stop', 'XAdsChains', 1, uout)

contains

  ! function AdsorbedChainFraction(ic, adsp, adsdist) result(adsfrac)

  !   ! Checks if chain ic is adsorbed to whatever particle is given by ads

  !   integer(4), intent(in) :: ic, adsp
  !   real(8),    intent(in) :: adsdist
  !   integer(4)  :: ip, iseg
  !   real(8)     :: dx, dy, dz, r2, adsfrac
  ...
  !      ip = ipnsegcn(iseg, ic)
  !      dx = ro(1,ip)-ro(1,adsp)
  !      dy = ro(2,ip)-ro(2,adsp)
  !      dz = ro(3,ip)-ro(3,adsp)
  !      call PBCr2(dx,dy,dz,r2)

  !      if (r2 < adsdist**2) then
  !         adsfrac = adsfrac+1
  !      end if
  !   end do

  !   adsfrac = adsfrac/npct(ictcn(ic))

  ! end function AdsorbedChainFraction

  function ChainIsAdsorbed(ic, adsp, adsdist) result(lads)

    ! Checks if chain ic is adsorbed to whatever particle is given by adsp
  ...
  end function ChainIsAdsorbed

end subroutine XAdsChains



subroutine MeanDist(iStage)
  ...
  character(40), parameter :: txroutine ='MeanDist'
  character(80), parameter :: txheading ='Mean distance between particles of specific type, dep. on adsorbed chains'
  !type(scalar_var), allocatable, save :: fca(:)    ! fraction of chains adsorbed
  type(scalar_var), allocatable, save :: md(:)    ! mean distance  
  integer(4), save         :: adspt, npa
  real(8), save            :: adsdist
  real(8)                  :: dx, dy, dz, r2, tmd
  integer(4)               :: ic, ict, ip, iseg, ipa, jp, npairs
  character(7)             :: temp_string

  namelist /nmlMeanDist/ adspt, adsdist
  ! adspt: particle type chains adsorb to, and whos separation we measure
  ! adsdist: adsorption distance
  ...
  if(iStage == iReadInput) then
     ...
     rewind(uin)
     read(uin,nmlMeanDist)


  else if(iStage == iWriteInput) then

     npa = nppt(adspt)+2   ! number of particles to adsorb on + 2 for zero adsorption and total
     allocate(md(npa*nct))

     do ict = 1, nct
        do ipa = 1, npa
           write(temp_string, '(a6,i1)') 'X_ads_', ipa-1
           md( (ict-1)*npa + ipa )%label = temp_string !//' ('//trim(txct(ict))//')'
        end do
        md( ict*npa )%label = 'X_ads_tt'
     end do

     
  else if(iStage == iBeforeSimulation) then

     if(lPBC) call Warn(txroutine, 'Routine not tested for PBC', uout)
     
     call ScalarSample(iStage, 1, nct*npa, md)   
     if(lsim .and. master .and. txstart == 'continue') read(ucnf) md(:)
     ...
     npairs = 0
     do ip = ipnpt(adspt), ipnpt(adspt) + nppt(adspt)-1
        do jp = ip+1, ipnpt(adspt) + nppt(adspt)-1
           dx = ro(1,ip) - ro(1,jp)
           dy = ro(2,ip) - ro(2,jp)
           dz = ro(3,ip) - ro(3,jp)
           call PBCr2(dx,dy,dz,r2)
     ...
     do ic = 1, nc
        ...
        md( (ict-1)*npa+ipa )%value = md( (ict-1)*npa+ipa )%value + tmd
        md( ict*npa )%value = md( ict*npa )%value + tmd  ! total
        call ScalarSample(iStage, (ict-1)*npa+ipa, (ict-1)*npa+ipa, md)
        call ScalarSample(iStage, ict*npa, ict*npa, md)        
     end do
     ...
     call ScalarSample(iStage, 1, nct*npa, md)
     if(lsim .and. master) write(ucnf) md
     ...
     do ict = 1, nct
        write(uout, '(a)') ''
        write(uout,'(2a)') 'chain type: ', trim(txct(ict))
        call ScalarWrite(iStage, (ict-1)*npa+1, ict*npa, md((ict-1)*npa+1:ict*npa), 1, '(a,t35,4f15.3,f15.0)', uout)
        write(uout,'(a)')  '--------'
     end do
  ...
  call CpuAdd('stop', 'MeanDist', 1, uout)
...
end subroutine MeanDist





subroutine AminoCharge(iStage)

  ! Routine to check the charge state of different amino groups (primary-tertiary) in branched PEI
  ...
  character(40), parameter :: txroutine ='AminoCharge'
  character(80), parameter :: txheading ='Fractional charge for amino groups'
  type(scalar_var), allocatable, save :: var(:)
  integer(4), allocatable, save :: ag(:,:), nn(:) ! List over particles w. primary, secondary and tertiary amino groups.
  integer(4), save         :: pt  ! particle type w. amino group
  integer(4)               :: i, j, ic, ict, ip, iseg, neighbours
  integer(4), allocatable  :: temp_arr(:), ind(:)   ! only used in WriteInput stage

  namelist /nmlAminoCharge/ pt
  ...
  case (iReadInput)
     ...
     rewind(uin)
     read(uin,nmlAminoCharge)

     if (pt .lt. 1 .or. pt .gt. npt) then
        call Stop(txroutine, 'Invalid particle type', uout)
     end if
        
  case (iWriteInput)

     ! Create lists of particle number for the different groups, rather
     ! than looping over and checking which group they are in the simulation step
     allocate(temp_arr( ipnpt(pt):ipnpt(pt)+nppt(pt)-1 ))
     allocate(nn(3), ind(3))
     temp_arr = 0
     nn = 0
     ind = 1

     do ip = ipnpt(pt), ipnpt(pt)+nppt(pt)-1
        neighbours = 2 + nbondcl(ip)
        ic = icnpn(ip)
        iseg = isegpn(ip)
        
        if ((iseg .eq. 1) .or. (iseg .eq. npct(ictcn(ic)))) then  ! At the end of chain
           neighbours = neighbours - 1
        end if
        temp_arr(ip) = neighbours
        nn(neighbours) = nn(neighbours) + 1
        if(neighbours < 1 .or. neighbours > 3) then
           write(uout, '(a, i4)') 'Number of monomer neighbours: ', neighbours
           call Stop(txroutine, 'Number of monomer neighbours are less than 1 or more than 3', uout)
        end if
     end do

     allocate(ag(maxval(nn(:)), 3))
     ag = 0
     
     do ip = ipnpt(pt), ipnpt(pt)+nppt(pt)-1
        i = temp_arr(ip)
        ag(ind(i), i) = ip
        ind(i) = ind(temp_arr(ip)) + 1
     end do
     ...
     var(1)%label = 'primary groups  '
     var(2)%label = 'secondary groups'
     var(3)%label = 'tertiary groups '


  case (iBeforeSimulation)
     
     call ScalarSample(iStage, 1, 3, var)
     if(lsim .and. master .and. txstart == 'continue') read(ucnf) var
  ...
  case (iSimulationStep)
     ...
     do i = 1, 3
        do j = 1, nn(i)
           ip = ag(j, i)
           if (laz(ip)) var(i)%value = var(i)%value + abs(az(ip))
        end do
        var(i)%value = var(i)%value/nn(i)
     end do
  ...
  case (iAfterMacrostep)
     ...
     call ScalarSample(iStage, 1, 3, var)
     if(lsim .and. master) write(ucnf) var(1:3)
     
  case (iAfterSimulation)
     
     call ScalarSample(iStage, 1, 3, var)
     call WriteHead(2, txheading, uout)
     call ScalarWrite(iStage, 1, 3, var, 1, '(a,t35,4f15.3,f15.0)', uout)
...
end subroutine AminoCharge



!************************************************************************
!*                                                                      *
!*     ChainBeadPartContactBranched                                     *
!*                                                                      *
!************************************************************************

! ... calculate probability of chain bead-particle contact of branched chains
! ... taken from ChainBeadPartContact in static.F90

   subroutine ChainBeadPartContactBranched(iStage)
   ...
   character(40), parameter :: txroutine ='ChainBeadPartContactBranched'
   character(80), parameter :: txheading ='probability of chain bead-particle contacts'
   integer(4)   , parameter :: ntype = 10
   integer(4),   allocatable,  save :: nbin(:)
   integer(4),                 save :: nvar
   ...
   namelist /nmlCBPCB/ iptpart, rcontact
   ...
   case (iWriteInput)
      ...
      allocate(nbin(nct))
      nbin(1:nct) = npct(1:nct)
      if ( any(nbin > mnbin_df) ) call Stop(txroutine, 'nbin > mnbin_df', uout)
...
! ... set ipnt, label, min, max, and nbin
      ...
      do ic = 1, nc                         ! loop over all chains
         ...
         do iploc = 1, nppt(iptpart)        ! loop over all particles of type iptpart
            ...
            var(ivar)%label = 'c.'//trim(adjustl(str1))//' p.'//trim(adjustl(str2))//' ct.'//trim(adjustl(txct(ict)))
            var(ivar)%min = 1-0.5
            var(ivar)%max = npct(ict)+0.5
            var(ivar)%nbin = nbin(ict)
         end do
      end do
      call DistFuncSample(iStage, nvar, var)
...
end subroutine ChainBeadPartContactBranched
