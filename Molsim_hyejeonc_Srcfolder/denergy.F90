...
subroutine DUExternalCylShell
! routine to set one hard cylinder shell, centered with the cell. 
   real(8) :: r2new

!print*,'#### In DUExternalCylShell ####'
!print*,'rcap, dcap, lcap', rcap, dcap, lcap
!print*,'inner and outer limits', (rcap-radat(ipt))**2, (rcap+dcap+radat(ipt))**2
   do ia=ianpn(ip),ianpn(ip)+napt(ipt)-1
      ialoc=ialoc+1
      r2new = rtm(1,ialoc)**2+rtm(2,ialoc)**2
!print*,'coordinates and r2new', rtm(1,ialoc), rtm(2,ialoc), rtm(3,ialoc), r2new
      if ((rtm(3,ialoc)+radat(ipt)) >= half*lcap .or. (rtm(3,ialoc)-radat(ipt)) <= -half*lcap) cycle
      if (r2new >= (rcap-radat(ipt))**2 .and. r2new <= (rcap+dcap+radat(ipt))**2) then
!print*,'overlaping'
         lhepoverlap = .true.
         exit
      end if
   end do
end subroutine DUExternalCylShell
